    ...
    if (invalidSpecialities.length) {
      throw new AppError(
        `Specialities ${JSON.stringify(invalidSpecialities)
          .split('"')
          .join('')} doesn't exists in this doctor`,
      );
    }

    if (specialitiesInDoctor.length - speciality_ids.length < 2) {
      throw new AppError('A doctor must have at least 2 specialities');
    }

    const savedDoctor = await this.doctorsRepository.removeDoctorsSpecialities(
      doctor,
      speciality_ids,
    );
